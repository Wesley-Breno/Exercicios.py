"""
Decorator é um padrão de projeto estrutural que permite que você adicione novos comportamentos em objetos ao coloca-los
dentro de um 'wrapper' (decorador) de objetos.
Decoradores fornecem uma alternativa flexivel ao uso de subclasses para a extensão de funcionalidades.

Decorator (padrão de projeto != Decorator em Python

Python decorator -> Um decorator é um callable que aceita outra função como argumento (a função decorada). O decorator
pode realizar algum processamento com a função decorada e devolve-la ou substitui-la por outra função ou objeto
invocavel.

Do livro 'Python Fluente', por Luciando Ramalho (pag. 223)
"""
