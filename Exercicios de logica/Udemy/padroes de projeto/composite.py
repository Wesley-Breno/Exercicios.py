"""
Composite é um padrão de projeto estrutural que permite que voce utilize a composição para criar objetos em estruturas
de arvores. O padrão permite aos clientes tratarem de maneira uniforme objetos individuais (Leaf) e composições de
objetos (Composite).

IMPORTANTE: só aplique este padrão em um estrutura que possa ser representada em formato hierarquico (arvore).

No padrão composite, temos dois tipos de objetos:

    Composite
        que representa nós internos da arvore.

    LeaF
        que representa nós externos da arvore.

Objetos Composite são objetos mais complexos e com filhos. Geralmente, eles delegam trabalho para os filhos usando um
metodo em comum.

Objetos LeaF são objetos simples, da ponta e sem filhos. Geralmente, são esses objetos que realizam o trabalho real da
aplicação.
"""
...
if __name__ == '__main__':
    # Leaf
    camiseta1 = Product('camiseta1', 39.3)
    camiseta2 = Product('camiseta2', 29.2)
    camiseta3 = Product('camiseta3', 19.1)
