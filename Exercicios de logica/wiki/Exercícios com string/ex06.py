...
def data_com_mes_por_extenso(data: str) -> Union[str, None]:
    ...
    if data_valida:
        return f'Você nasceu em {valores[0]} de {mes_por_extenso} de {valores[2]}'


if __name__ == '__main__':
    while True:  # Enquanto o usuario nao informar a data de nascimento corretamente
        data_extenso = data_com_mes_por_extenso(str(input('Data de nascimento: ')))

        if data_extenso is not None:  # Se o usuario informou a data corretamente
            print(data_extenso)
            break

        else:
            print('\n\n\t\t[ERRO]: Informe a sua data de nascimento corretamente.\n\n')
